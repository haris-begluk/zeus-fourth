    ...
    public class MojDBInitializer
    {
        public static void Izvrsi(MojContext context)
        {
            ...
            for (int i = 1; i <= maxRazredi; i++)
            {
                ...
                {
                    predmeti.Add(new Predmet { Naziv = MyRandomExtensions.MyRandomString(5), Razred = i });
                }
            }

            skolskeGodine.Add(new SkolskaGodina { Aktuelna = false, Naziv = "2016/17" });
            skolskeGodine.Add(new SkolskaGodina { Aktuelna = true, Naziv = "2017/18" });
            ...
            var tpredmeti = context.Predmet.ToList();
            var tskole = context.Skola.ToList();
            var tnastavnik = context.Nastavnik.ToList();
            var todjeljenje = context.Odjeljenje.ToList();
            for (int i = 0; i < 80; i++)
            {
                context.OdrzanCas.Add(new OdrzanCas
                {
                    Datum = DateTime.Now,
                    PredmetId = tpredmeti.MyRandom().Id,
                    SkolaId = tskole.MyRandom().Id,
                    NastavnikId = tnastavnik.MyRandom().Id,
                    OdjeljenjeId = todjeljenje.MyRandom().Id,
                    SadrzajCasa = "Cas je odrzan u najboljem redu, na unaprijed odredjenu tematiku."
                });
            }
            context.SaveChanges();
            var todrzanicasovi = context.OdrzanCas.ToList();
            var todjeljenjestavka = context.OdjeljenjeStavka.ToList();
            for (int i = 0; i < 200; i++)
            {
                context.OdrzanCasDetalji.Add(new OdrzanCasDetalji
                {
                    OdjeljenjeStavkaId = todjeljenjestavka.MyRandom().Id,
                    OdrzanCasId = todrzanicasovi.MyRandom().Id,
                    Ocjena = MyRandomExtensions.dajOcjenu(),
                    Napomena = "Napomena jako vazna",
