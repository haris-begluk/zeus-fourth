...
namespace RS1_Ispit_asp.net_core.EF
{
    public class MojContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<MaturskiIspit>().HasOne(x => x.Odjeljenje)
                .WithMany().OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MaturskiIspitStavka>().HasOne(x => x.OdjeljenjeStavka)
                .WithMany().OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<OdrzanCas>().HasOne(x => x.Odjeljenje)
                .WithMany().OnDelete(DeleteBehavior.Restrict);
            modelBuilder.Entity<OdrzanCasDetalji>().HasOne(x => x.OdrzanCas)
                .WithMany().OnDelete(DeleteBehavior.Restrict);
        }
        ...
        public DbSet<OdrzanCas> OdrzanCas { get; set; }
        public DbSet<OdrzanCasDetalji> OdrzanCasDetalji { get; set; }
    }
}